...
/*void drawFrame(int h, int w, unsigned char* pixels){
	setAddrWindow(0, 0, w-1, h-1);
	for(int c=159; c>=0; c--){
		for(int r=127; r>=0; r--){
			char pixel = pixels[r*160 + c];
			uint16_t pixel_16 = getColor_16(pixel);
			writedata((uint8_t)(pixel_16 >> 8));
			writedata((uint8_t)(pixel_16));
		}
	}
}*/

//-----------drawFrame-------------
// Draws an array of 8 bit colors to the screen. Assumes 160x128 array size.
// Input: pointer to array of pixels in 8 bit color
// Output: 16-bit color drawn to screen
void drawFrame(int h, int w, unsigned char* pixels){
  setAddrWindow(0, 0, ST7735_TFTWIDTH-1, ST7735_TFTHEIGHT-1);
  int skip = false;
  for(int c=ST7735_TFTHEIGHT-1; c>=0; c--){
		for(int r=ST7735_TFTWIDTH-1; r>=0; r--){
      char pixel = pixels[r*ST7735_TFTHEIGHT + c];
      if(getRefresh(pixel)){
        if(skip == true){
          skip = false;
          writecommand(ST7735_CASET); // Column addr set
          writedata(0x00);
          writedata(c+ColStart);     // XSTART
          writecommand(ST7735_RASET); // Row addr set
          writedata(0x00);
          writedata(r+RowStart);     // YSTART
        }
        uint16_t pixel_16 = getColor_16(pixel);
        pushColor(pixel_16);
      } else {
